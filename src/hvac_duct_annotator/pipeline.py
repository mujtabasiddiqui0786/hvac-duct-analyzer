...
from hvac_duct_annotator.classify import classify_segments_by_diffuser_adjacency, enrich_round_ducts_with_hough
...
def run_pipeline(
    ...
    segments, _graph = build_centerline_segments(lines=lines, circles=circles)
    ...
    associate_labels(segments, labels, z)

    scale = detect_scale_info(rendered.image_bgr, dpi=dpi)
    if scale_mode != "auto":
        scale.source = "manual"
